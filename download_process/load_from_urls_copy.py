    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'content-length' in resp.headers:
                ...
                # 尝试通过HEAD请求获取总大小
                try:
                    head_resp = requests.head(url, timeout=40)
...
def get_txt_path(root_dir):
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        if len(filenames) > 0:
            for fl in filenames:
                if fl.endswith('.txt') and fl.split('_')[1] == 'historical.txt' and fl.split('_')[0] in ['huss', 'uas', 'vas', 'tas']:
                    save_dir_load_txt[f'{dirpath}']=(os.path.join(dirpath,fl))
    return save_dir_load_txt

def main():
    save_dir_load_txt=get_txt_path(root_dir = 'G:\\MIROC-ES2H')
